...
Sections~\ref{sec:experimental_analysis} and~\ref{sec:application} documented four empirical patterns: early entropy rise, trajectory-level diversification, Pass@$N$ accumulation under independent draws, and a late-training DAPO advantage. We now explain these mechanistically. RSP enters the hidden state through self-attention, so within one attention head the magnitude envelope of its instantaneous contribution is controlled by one scalar: the \emph{attention mass} $w$ on RSP tokens. Early on, $w$ is large enough to branch the hidden state into multiple directions (\S\ref{sec:improve}); as the KV cache grows, its upper bound narrows and the response converges (\S\ref{sec:converge}). Randomness supplies \emph{independent} perturbations across rollouts so each opens a fresh branch (RSP definition and \emph{branching event/state} terminology from \S\ref{sec:rsp_definition}; further proofs in Appendix~\ref{app:proofs}).
