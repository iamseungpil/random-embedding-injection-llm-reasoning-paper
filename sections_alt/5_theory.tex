...
Table~\ref{tab:performance}'s headline --- random embeddings without training already lift reasoning accuracy --- raises one question: \emph{why} does an untrained random vector help? \S\ref{sec:experimental_analysis}--\S\ref{sec:application} added four mechanistic patterns: early entropy rise, trajectory-level diversification, Pass@$N$ accumulation under independent draws, and a late-training DAPO advantage. RSP enters the hidden state through self-attention, so within one head the magnitude of its instantaneous contribution reduces to one scalar: the \emph{attention mass} $w$ on RSP tokens. Early in decoding $w$ is large enough to branch hidden states into multiple directions (\S\ref{sec:improve}); as the KV cache grows, the upper bound on $w$ narrows automatically and the model commits to the branch already opened (\S\ref{sec:converge}) --- an implicit explore-then-exploit. Independent resampling per rollout opens a fresh branch each time (\S\ref{sec:rsp_definition} for RSP definition and \emph{branching event/state} terminology; proofs in Appendix~\ref{app:proofs}).
...
\begin{figure}[ht]
  ...
  \caption{Per-token attention mass on Qwen2.5-Math-7B (\emph{suffix}, 500 MATH-500 problems, independent RSP per problem). (a) RSP-token attention decreases along the reasoning-position axis, matching Theorem~\ref{thm:decay}'s bound; the additional layer-axis decrease is empirical and outside the theorem's scope. (b) Question-token attention stays comparatively uniform. Reported quantity: per-token mass (Appendix~\ref{app:attention_mass}), $w_{r,i}^{(\ell)}/L$ averaged over heads and samples.}
  \label{fig:attention_mass}
\end{figure}

Lifting single-shot branching to task accuracy across $N$ rollouts requires (M1) marginal $p_{\min}(x)>0$ for a correct trajectory and (M2) mutual independence. Under both, $\Pr(\mathrm{Pass@}N \text{ on } x) \ge 1 - (1-p_{\min}(x))^N$ (Appendix~\ref{app:passN_ensemble}). \S\ref{sec:improve} supports the branch-opening mechanism behind (M1) without guaranteeing correctness; \emph{independent resampling} supplies (M2). Sharing one RSP breaks (M2) and removes the accumulation --- the signature \S\ref{sec:diversity} documents empirically.
