...
For the analysis in \S\ref{sec:why_rsp_works}, we will call decoding step $t$ a \emph{branching event}, and the corresponding hidden state $h^{(t)}$ a \emph{branching state}, when the next-token top-$K$ set produced by $h^{(t)}$ differs from the top-$K$ produced by the no-RSP baseline $\bar h^{(t)}$ (the red dots of Figure~\ref{fig:main}).\footnote{$K$ covers any candidate-set definition, including top-$K$ and top-$p$ nucleus. We reserve $t$ for decoding step and $\ell$ for self-attention layer throughout.}
...
\begin{table}[!t]
\centering
\caption{Accuracy (\%) across model configurations and benchmarks. Values in parentheses indicate the difference from the baseline. Full per-position breakdown (including infix) is in Appendix~\ref{app:ablation}.}
...
We first examine how injecting random embeddings affects reasoning performance. Table~\ref{tab:performance} reports accuracy at the Prefix and Suffix positions (the full per-position breakdown including infix is in Appendix~\ref{app:ablation}).
