...
For the analysis in \S\ref{sec:why_rsp_works}, we will call decoding step $\ell$ a \emph{branching event}, and the corresponding hidden state $h^{(\ell)}$ a \emph{branching state}, when the next-token top-$K$ set produced by $h^{(\ell)}$ differs from the top-$K$ produced by the no-RSP baseline $\bar h^{(\ell)}$ (the red dots of Figure~\ref{fig:main}).\footnote{$K$ covers any candidate-set definition, including top-$K$ and top-$p$ nucleus.}
