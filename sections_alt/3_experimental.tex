...
For the analysis in \S\ref{sec:why_rsp_works}, call decoding step $t$ a \emph{branching event} and $h^{(t)}$ a \emph{branching state} when the next-token top-$K$ set of $h^{(t)}$ differs from that of the no-RSP baseline $\bar h^{(t)}$ (red dots of Figure~\ref{fig:main}).\footnote{$K$ covers top-$K$/top-$p$ nucleus; $t$ for step, $\ell$ for layer.}
