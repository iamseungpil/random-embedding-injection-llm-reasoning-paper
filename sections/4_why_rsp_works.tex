...
RSP does not add to the hidden state directly but enters through self-attention, so within one attention head its instantaneous influence reduces to a single scalar: the \emph{attention mass} $w$ on RSP tokens. This section tracks how $w$ evolves during decoding. Early on $w$ is large enough to branch the hidden state into multiple directions (\S\ref{sec:improve}); as the KV cache grows, its upper bound narrows and the response converges (\S\ref{sec:converge}). Randomness supplies \emph{independent} perturbations across rollouts so that each opens a fresh branch. Further proofs are in Appendix~\ref{app:proofs}.

% ================================================================
\subsection{Random Soft Prompt}\label{sec:rsp_definition}
% ================================================================

We start with the object being analyzed. Let $\mathbf{W}_E \in \mathbb{R}^{V\times d}$ denote the pretrained token-embedding matrix, with $V$ the vocabulary size and $d$ the hidden dimension. Write its entrywise statistics as $\mu_E := \tfrac{1}{V\cdot d}\sum_{i,k}[\mathbf{W}_E]_{i,k}$ (the mean over all $V\cdot d$ entries) and $\sigma_E := \mathrm{std}(\mathbf{W}_E)$. An RSP of length $L$ is a sequence of continuous vectors $\mathbf{H} = [h_1,\dots,h_L]\in\mathbb{R}^{L\times d}$ drawn independently from
...
Call decoding step $t$ a \emph{branching event} and the last-layer hidden state $\mathbf{s}^{(t)}$ a \emph{branching state} when the LM-head top-$K$ differs from that of the no-RSP baseline $\bar{\mathbf{s}}^{(t)}$ (Figure~\ref{fig:main}, red).\footnote{Italic $h_j$ lives in the RSP input space; bold $\mathbf{s}^{(t)}$ is the model's hidden state. $K$ is the next-token sampler's candidate-set size, instantiated as either the fixed cutoff of top-$K$ truncation or the dynamic nucleus size of top-$p$ sampling. $t$ indexes decoding step, $\ell$ indexes self-attention layer.} \S\ref{sec:improve} asks \emph{when} a branching event is reachable, and \S\ref{sec:converge} asks \emph{how} that reachability fades.

% ================================================================
\subsection{Exploration: RSP strengthens early-stage exploration}\label{sec:improve}\label{sec:multipath}
% ================================================================

Inserting a random prompt concentrates attention on it, and that concentration is what amplifies exploration. The effect of a single RSP injection on the hidden state raises two questions: \emph{how much}, and \emph{in which direction}. The first is decided by a single scalar $w$ inside one attention head, the second by the distribution of $\mathbf{H}$. RSP's design choice, an isotropic Gaussian, addresses both at once.
...
Gaussian RSP satisfies the equality with $\Sigma = \sigma_E^2 \mathbf{I}_d$ (so $\rho^2 = d\sigma_E^2$) and adds \emph{full support} on $\mathbb{R}^d$ (Appendix~\ref{app:why_random}). Isotropy ensures no direction is systematically excluded; full support gives positive probability to the open branching-event region (\S\ref{sec:rsp_definition}) whenever it is non-empty (Appendix~\ref{app:topk_entry}). Monotone temperature preserves ranking and cannot reach this region, so RSP's \emph{rank-changing} branching follows from the two properties together, splitting the hidden state into multiple branching states early in reasoning.

% ================================================================
\subsection{Annealing: KV cache growth dilutes RSP attention}\label{sec:converge}
% ================================================================

The early branching that RSP introduces stabilizes naturally as decoding proceeds: the upper bound on the same scalar $w_{r,i}^{(\ell)}$ defined in \S\ref{sec:improve} narrows along the sequence axis with autoregressive KV cache growth. Each step appends one real-token term while the number of RSP terms is fixed; freezing the attention-logit gap between real and RSP tokens, this asymmetry yields the following exact bound.
