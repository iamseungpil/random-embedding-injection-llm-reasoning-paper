...
Call decoding step $t$ a \emph{branching event} and the last-layer hidden state $\mathbf{s}^{(t)}$ a \emph{branching state} when its top-$K$ differs from that of the no-RSP baseline $\bar{\mathbf{s}}^{(t)}$ (Figure~\ref{fig:main}, red).\footnote{Italic $h_j$ lives in the RSP input space; bold $\mathbf{s}^{(t)}$ is the model's hidden state. $K$ is the next-token sampler's candidate-set size, instantiated as either the fixed cutoff of top-$K$ truncation or the dynamic nucleus size of top-$p$ sampling. $t$ indexes decoding step, $\ell$ indexes self-attention layer.} \S\ref{sec:improve} asks \emph{when} a branching event is reachable, and \S\ref{sec:converge} asks \emph{how} that reachability fades.
