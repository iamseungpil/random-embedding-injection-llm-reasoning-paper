...
Call decoding step $t$ a \emph{branching event}, and $h^{(t)}$ a \emph{branching state}, if the next-token top-$K$ set produced by $h^{(t)}$ differs from the top-$K$ produced by the no-RSP baseline $\bar h^{(t)}$ (the red dots of Figure~\ref{fig:main}).\footnote{$K$ covers any candidate-set definition, including top-$K$ and top-$p$ nucleus. We reserve $t$ for decoding step and $\ell$ for self-attention layer throughout.} \S\ref{sec:improve} asks \emph{when} a branching event is reachable, and \S\ref{sec:converge} asks \emph{how} that reachability fades.
...
Consider a single attention head where a query attends $n\ge1$ real tokens and $L\ge1$ RSP tokens with finite logits, and let $\Delta_i := \min_{j\in[n]}\mathbf{q}_i^\top\mathbf{k}_j - \max_{j'\in[L]}\mathbf{q}_i^\top\mathbf{k}_{r_{j'}}$ denote the pre-scale logit gap between real and random tokens. In scaled dot-product attention,
\[
...
\begin{proof}
Let $s_j := \mathbf{q}_i^\top\mathbf{k}_j/\sqrt d$, $s_{r_{j'}} := \mathbf{q}_i^\top\mathbf{k}_{r_{j'}}/\sqrt d$, $s_{r,\max} := \max_{j'} s_{r_{j'}}$, and $R := \sum_{j'=1}^L \exp(s_{r_{j'}})$. The gap definition gives $s_j \ge \Delta_i/\sqrt d + s_{r,\max}$ for every real $j$, and $\exp(s_{r,\max}) \ge R/L$ holds because the max dominates the average; combining the two and summing over the $n$ real keys yields $\sum_{j=1}^n \exp(s_j) \ge (n/L)\exp(\Delta_i/\sqrt d)\,R$. Substituting into $w_{r,i} = R/(\sum_j\exp(s_j) + R)$ gives
\[
w_{r,i}
\;\le\;
