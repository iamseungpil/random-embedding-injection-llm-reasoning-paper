...
Consider a single attention head where query $\mathbf{q}_i$ at position $i$ attends $n\ge1$ real keys $\mathbf{k}_j$ and $L\ge1$ RSP keys $\mathbf{k}_{r_{j'}}$ under finite logits. Define the pre-scale logit gap $\Delta_i := \min_{j\in[n]}\mathbf{q}_i^\top\mathbf{k}_j - \max_{j'\in[L]}\mathbf{q}_i^\top\mathbf{k}_{r_{j'}}$. In scaled dot-product attention,
\[
