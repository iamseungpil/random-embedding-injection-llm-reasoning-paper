...
Call decoding step $t$ a \emph{branching event} and the last-layer hidden state $\mathbf{s}^{(t)}$ a \emph{branching state} when the LM-head top-$K$ differs from that of the no-RSP baseline $\bar{\mathbf{s}}^{(t)}$ (Figure~\ref{fig:main}, red).\footnote{Italic $h_j$ lives in the RSP input space; bold $\mathbf{s}^{(t)}$ is the model's hidden state. $K$ covers top-$K$/top-$p$ nucleus. $t$ for decoding step, $\ell$ for self-attention layer.} \S\ref{sec:improve} asks \emph{when} a branching event is reachable, and \S\ref{sec:converge} asks \emph{how} that reachability fades.
