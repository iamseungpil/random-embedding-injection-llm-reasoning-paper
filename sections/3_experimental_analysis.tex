...
We examine how the \emph{early hidden-state branching followed by cache-growth stabilization} pattern of Figure~\ref{fig:main} surfaces in LLM generation through three scenes: whether RSPs preserve baseline accuracy and even recover it in some settings (\S\ref{sec:performance}); whether the early diversification and later stabilization suggested by the attention decomposition and the cache-growth envelope are observed in practice (\S\ref{sec:entropy}); and whether Pass@$N$ depends on \emph{independence} of the RSP draw across samples (\S\ref{sec:diversity}).
