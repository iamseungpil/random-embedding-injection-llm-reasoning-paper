...
We now examine how the explanation in §\ref{sec:why_rsp_works} appears in actual LLM generation through three scenes. First, we ask whether RSP preserves baseline accuracy without breaking it, and recovers it in some settings (\S\ref{sec:performance}). Second, we check whether the early diversification followed by late stabilization---predicted by the two-sided envelope---is observed empirically (\S\ref{sec:entropy}). Finally, we contrast Pass@$N$ between sharing one RSP across samples versus drawing a fresh RSP per sample, directly probing whether \emph{independence} is the key (\S\ref{sec:diversity}).
