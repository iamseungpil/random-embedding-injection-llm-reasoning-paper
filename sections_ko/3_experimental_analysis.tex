...
§\ref{sec:why_rsp_works}의 분석은 세 가지 검증 가능한 예측을 만든다. (P1) RSP는 baseline 정확도를 크게 깎지 않으면서 일부 설정에서 회복시킨다 — 단발 RSP가 정답 trajectory를 완전히 망가뜨릴 만큼 큰 섭동이 아니라는 Theorem~\ref{thm:decay}의 귀결. (P2) 생성 초반에 토큰 분포가 평탄해졌다가 후반에 baseline 수준으로 돌아온다 — 같은 정리에서 따라오는 자동 어닐링의 직접 흔적. (P3) 독립 RSP 추첨이 Pass@$N$을 단조롭게 끌어올리고, 그 한계는 단일 고정 RSP를 재사용했을 때보다 본질적으로 크다 — Proposition~\ref{prop:multi_path}이 예측하는 지수 saturation이 실제로 작동한다는 신호. 본 절은 세 예측을 차례로 확인한다.
