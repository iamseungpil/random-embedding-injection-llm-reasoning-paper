...
앞 절의 설명이 실제 LLM 생성에서 어떻게 드러나는지를 세 장면으로 살펴본다. 먼저 RSP가 baseline 정확도를 크게 깎지 않으면서 일부 설정에서는 오히려 회복시키는지를 확인하고(\S\ref{sec:performance}), 다음으로 생성 초반에 토큰 분포가 평탄해졌다가 후반에는 baseline 수준으로 돌아오는 자동 어닐링이 실제로 관찰되는지를 살피며(\S\ref{sec:entropy}), 끝으로 같은 RSP를 재사용할 때와 매 시도마다 새로 뽑을 때의 Pass@$N$ 차이가 \emph{독립성}이 핵심이라는 점을 직접 보여 주는지를 본다(\S\ref{sec:diversity}).
