...
\begin{wraptable}{r}{0.55\textwidth}
  ...
  \end{tabular}
\end{wraptable}
먼저 토큰 수준에서 보면, 자기회귀 생성에서 baseline과 RSP의 궤적 차이는 첫 생성 step에서 시작되어야 한다. 이 위치가 추론 궤적의 핵심 분기점이기 때문이다 \citep{wang2025beyond}. Baseline $\tau{=}1.0$을 기준으로, 더 높은 temperature의 baseline $\tau{=}2.0,3.0$과 RSP $\tau{=}1.0$을 네 지표로 비교한다. Spearman rank correlation $\rho$는 순위 보존 여부를, baseline top-10 밖 확률 질량은 저확률 토큰으로의 support 확장을, Jensen--Shannon divergence는 전체 분포 거리를, Acc는 Pass@1 정확도를 측정한다(정의는 Appendix~\ref{app:token_metrics}).

Table~\ref{tab:rsp_vs_temp}의 대비는 명확하다. RSP의 분포 변화 크기는 top-10 밖 질량과 JS divergence 기준으로 $\tau{=}2.0$과 $\tau{=}3.0$ 사이에 놓인다. 그러나 temperature scaling은 단조 변환이라 어떤 $\tau$에서도 baseline 대비 token ranking을 보존하므로 $\rho=1$이 강제된다. 반면 RSP는 순위를 부분적으로 보존하면서도 재배치하여 $\rho=0.709$를 만든다. Acc 행은 그 비용 차이를 보여 준다. RSP와 비슷한 크기의 분포 변화를 temperature로 만들려면 $\tau$를 3.0 쪽으로 올려야 하지만, 이 경우 정확도는 1.42\%까지 무너진다. RSP는 같은 설정에서 73.30\%의 정확도를 유지한다. 따라서 RSP는 temperature처럼 분포를 균일하게 평탄화하는 것이 아니라, 순위를 바꾸는 다른 종류의 섭동을 만든다.
